...
if ( ! class_exists( 'Aora_Shop_WooCommerce' ) ) :
	...
	class Aora_Shop_WooCommerce  {
		...
		public function __construct() {
			...
			add_filter( 'aora_woo_config_product_layout', array( $this, 'product_layout_style') );
		...
		public function product_layout_style() {
	        $type_array   = apply_filters( 'aora_get_template_product', 10,1 );
	        $type = aora_tbay_get_config('product_layout_style', 'v1'); 

	        $type = (isset($_GET['product_layout_style'])) ? $_GET['product_layout_style'] : $type;
	        
	        if (!in_array( $type, $type_array)) $type = 'v1';
	  
	        if( apply_filters( 'aora_product_layout_mobile', wp_is_mobile() ) ) {
	            $type = 'v1';
	        }
